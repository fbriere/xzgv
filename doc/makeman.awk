...
# It only supports the bare minimum required by xzgv.texi.
...
# in xzgv.texi, this is likely to need fixing to cope with it. :-)
...
print ".TH XZGV 1 \"" var_value["UPDATED"] "\" " \
	"\"Version " var_value["VERSION"] "\" \"Graphics Software\""
print ".SH NAME"
print "xzgv \\- picture viewer for X, with thumbnail-based file selector"
print ".\\\""

print ".SH SYNOPSIS"
print "xzgv [\\fIoptions\\fR] [\\fIdir\\fR | \\fIfile\\fR ...]"
}
...
/^@(chapter|section|subsection|subsubsection)\>/ \
  ...
  sub(/^Invoking xzgv/,"OPTIONS",sec_name)
  ...
  if(sec_name=="DESCRIPTION")
    {
    print "(NB: This man page is automagically generated from xzgv's"
...
/^@titlepage\>/ \
  {
  # given the way xzgv.texi is laid out, we know VERSION and
...
(/^$/ && !last_was_blank) || was_item || was_tableend \
  ...
  # @var{dir|file} -> italic dir|file
  para=gensub(/@var\{(dir|file)\}/, "\\\\fI\\1\\\\fR", "g", para)
  ...
  # finally, a bit of a kludge - replace "\fBInvoking xzgv\fR" with
  # "\fBOptions\fR", assuming it's in a @pxref/@xref/@ref.
  para=gensub(/\\fBInvoking xzgv\\fR/, "\\\\fBOptions\\\\fR", "g", para)
