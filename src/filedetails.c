...
gtk_box_pack_start(GTK_BOX(gtk_dialog_get_content_area(GTK_DIALOG(details_win))),
                   vbox,TRUE,TRUE,0);
gtk_widget_show(vbox);

gtk_container_set_border_width(GTK_CONTAINER(vbox),5);
gtk_container_set_border_width(
  GTK_CONTAINER(gtk_dialog_get_action_area(GTK_DIALOG(details_win))),5);

/* add ok button */
action_tbl=gtk_table_new(1,5,TRUE);
gtk_box_pack_start(GTK_BOX(gtk_dialog_get_action_area(GTK_DIALOG(details_win))),
